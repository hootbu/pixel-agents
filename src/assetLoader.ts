...
export async function loadFurnitureAssets(
  ...
  try {
        ...
        for (const asset of assets) {
          try {
            ...
            if (!fs.existsSync(assetPath)) {
              console.warn(
                `  ⚠️  Asset file not found: ${asset.file} in ${dir.name}`,
              );
              continue;
            }
    ...
    console.log(
      `[AssetLoader] ✅ Successfully loaded ${sprites.size} furniture sprites`,
    );
...
export async function loadFloorTiles(
  ...
  try {
    ...
    console.log(
      `[AssetLoader] ✅ Loaded ${sprites.length} floor tile patterns from floors/`,
    );
    return { sprites };
  } catch (err) {
    console.error(
...
export function sendFloorTilesToWebview(
  ...
  console.log(
    `📤 Sent ${floorTiles.sprites.length} floor tile patterns to webview`,
  );
}
...
export function sendCharacterSpritesToWebview(
  ...
  console.log(
    `📤 Sent ${charSprites.characters.length} character sprites to webview`,
  );
}
